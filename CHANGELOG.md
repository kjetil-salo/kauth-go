...
- Spec-komplett OIDC `authorization_code`-flyt med PKCE (RFC 6749 §4.1 + RFC 7636), i tillegg til den eksisterende bespoke login/dispatch-flyten. `/login` fungerer nå også som `authorization_endpoint` for standard OIDC-klienter: en forespørsel med `response_type=code&client_id=...` valideres (kjent client_id, registrert redirect_uri, code_challenge påkrevd hvis tjenesten har `requires_pkce=1`) og bæres videre gjennom innloggingsreisen i en ny `oidc_authz`-cookie. `/dispatch` utsteder deretter en kortlevd, engangsbrukt autorisasjonskode (ny `authorization_codes`-tabell, 60s levetid) i stedet for et token i URL-en, og redirecter med `?code=&state=`. `/token` støtter en ny `grant_type=authorization_code`: koden løses inn (redirect_uri og client_id må matche eksakt hva koden ble utstedt for), PKCE verifiseres (`code_verifier` → SHA-256 → base64url mot lagret `code_challenge`), og et ekte `id_token` utstedes (med `aud`=client_id og `nonce` speilet fra requesten — atskilt fra det eksisterende access-tokenet, som aldri har hatt disse claimsene). Discovery-dokumentet lister nå `grant_types_supported`, `code_challenge_methods_supported` og `scopes_supported`. Ny `requires_pkce`-avkrysning i admin-panelets tjenesteskjema markerer en tjeneste som en offentlig klient uten client secret (typisk en ekstern partner-app uten egen backend). Motivert av et konkret behov: en ekstern tredjepart skal kunne implementere "Logg inn med [tjeneste]" med et standard OIDC-klientbibliotek, noe den eksisterende flyten (token direkte i URL, ingen `aud`/`nonce`) ikke støtter trygt utenfor tjenester vi selv kontrollerer klientkoden til.
